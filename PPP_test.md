---
marp: true
backgroundImage: url('/home/josh/Nextcloud/School/Spring_2026/CSC_494/CircuitBoard-LtGrey-Wide.jpg')
---
...
**Presentation Type:** Initial Project Kickoff (One-Time)

---

## Executive Summary

This project aims to create a local, self-hosted smart home control center using a Raspberry Pi and ESP32-C6 to address critical shortcomings in modern smart home ecosystems. The system will leverage Zigbee protocol to provide reliable, cloud-independent home automation with unified control.

---

## Problem Statement: Modern Smart Home Challenges

### Critical Issues with Current Smart Home Devices

1. **Cloud Dependency**
   - Devices become inoperable when internet connection fails
   - Privacy concerns with continuous data transmission to third-party servers
   - Service discontinuation renders hardware obsolete
   - Latency issues due to round-trip cloud processing

2. **Poor Integration**
   - Fragmented ecosystems prevent cross-brand communication
   - No unified control interface across manufacturers
   - Limited interoperability between devices
   - Vendor lock-in restricts expansion options

3. **App Proliferation**
   - Separate apps required for each manufacturer's devices
   - Inconsistent user interfaces and experiences
   - Increased cognitive load managing multiple applications
   - No centralized dashboard for comprehensive control

4. **WiFi Bandwidth Saturation**
   - Each device consumes WiFi bandwidth
   - Network congestion with multiple smart devices
   - Interference and connection reliability issues
   - Router limitations on concurrent device connections
...
│                   Raspberry Pi OS                       │
│  ┌───────────────────────────────────────────────────┐  │
│  │     Python3 + TKinter Control Interface           │  │
│  └───────────────────────────────────────────────────┘  │
│           ↓                    ↓                        │
│  ┌──────────────┐    ┌──────────────────────┐           │
│  │ DHT22 Sensor │    │   LD2410C Presence   │           │
│  │  (Temp/Hum)  │    │      Sensor          │           │
│  └──────────────┘    └──────────────────────┘           │
│  ┌──────────────────────────────────────────┐           │
│  │   Pi Camera Module v2 (9132664)          │           │
│  └──────────────────────────────────────────┘           │
└─────────────────────────────────────────────────────────┘
                         ↓
...
│  • LED Light Strips                                     │
│  • Smart Vent (Temperature-Controlled)                  │
│  • Future expandable devices                            │
└─────────────────────────────────────────────────────────┘
```
